---
title: C++第六章 STL Algorithms + Lambda
...
## 5. iterator 迭代器
> 基本上更安全的指標，常常拿來配合STL algorithms
...
- 說明：
    ...
    v.begin()  → 一個指向「第一個元素」的 iterator
    v.end()    → 一個指向「最後一個元素的下一格」的 iterator(不是最後一個!)
    ```
    實際上的指法：
    ```
    v = { 10, 20, 30 }
          ↑           ↑
       begin()      end()(指向第三個之後,一個「不存在」的位置)
    ```

    很多時候基本上就是當作指標在操作，只是也可以操作在 "不是連續記憶體" 的物件。
- 練習：用 iterator 風格手寫一個迴圈(不用 range-based for,逼自己用 `*it`/`it++`)

    - **寫一段程式:`std::vector<int> v = {3, 7, 1, 9, 4};`,用 iterator 風格的 for 迴圈(`for (auto it = v.begin(); it != v.end(); ++it)`)把每個元素印出來,並順便算出總和印在最後。**

    - 提示:總和要在迴圈外先宣告一個 `int sum = 0;`,迴圈裡 `sum = sum + *it;`。試試看。